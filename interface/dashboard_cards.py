...
<style>
    ...
    .card-loteria {
        ...
        margin-bottom: 24px; display: flex; flex-direction: column; height: 100%;
        transition: transform 0.2s; overflow: hidden;
    }
    .card-loteria:hover { transform: translateY(-5px); box-shadow: 0 12px 20px rgba(0,0,0,0.1); border-color: #cbd5e1; }

    .card-header { padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; background: #f8fafc; border-bottom: 1px solid #e2e8f0; }
...
def formatar_moeda(valor):
    try:
        if isinstance(valor, (int, float)): v_float = float(valor)
        else: v_float = float(str(valor).replace("R$", "").replace(".", "").replace(",", "."))
        return f"R$ {v_float:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    except: return "R$ ---"

def gerar_html_card(nome_loteria, motor, valor_api=None):
    ...
    try:
        if motor.df is not None and not motor.df.empty:
            ...
            # PRIORIDADE 1: API
            if valor_api and valor_api > 0:
                premio = formatar_moeda(valor_api)
            else:
                # PRIORIDADE 2: Planilha
    ...
    <div class="card-loteria">
        ...
        <div class="last-result-section">
            ...
            <div class="balls-grid">{html_bolas}</div>
        </div>
...
def gerar_ticket_visual(nome_loteria, numeros):
    ...
    <div class="ticket-container">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center;">{html_bolas}</div>
    </div>
    """
